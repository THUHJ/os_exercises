...
parser.add_option("-r", "--privilege", help="number of privilege", default=4,
                  action="store", type="int", dest="privilege")
...
if options.solve == True:
    ...
    if options.policy == 'MLFQ':
        ...
        wait = {}
        quantum  = float(options.quantum)
        privilege =  int(options.privilege)
        jobcount = len(joblist)
        for i in range(0,jobcount):
            lastran[i] = 0.0
            wait[i] = 0.0
        ...
        curr_priv = 0
        while (curr_priv<4):
            while len(runlist[curr_priv]) > 0:
                # print '%d jobs remaining' % jobcount
                job = runlist[curr_priv].pop(0)
                jobnum  = job[0]
                runtime = float(job[1])
                if response[jobnum] == -1:
                    response[jobnum] = thetime
                ...
                if runtime > quantum:
                    ...
                    runtime -= quantum
                    print '  [ time %3d ] Run job %3d for %.2f secs' % (thetime, jobnum, ranfor)
                    if curr_priv==privilege-1:
                        runlist[curr_priv].append([jobnum, runtime])
                    else:
                        runlist[curr_priv+1].append([jobnum, runtime])
                else:
                    #YOUR CODE
                    ranfor = runtime
                    print '  [ time %3d ] Run job %3d for %.2f secs ( DONE at %.2f )' % (thetime, jobnum, ranfor, thetime + ranfor)
    ...
    if options.policy != 'MLFQ' and options.policy != 'FIFO' and options.policy != 'SJF' and options.policy != 'RR' and options.policy!='STRIDE': 
