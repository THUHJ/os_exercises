...
>  口通信可以分为同步通信和异步通信两类。同步通信是按照软件识别同步字符来实现数据的发送和接收，异步通信是一种利用字符的再同步技术的通信方式。串口访问主要包括打开串口、读/写串口，关闭串口几个过程。例如linux系统下，串口通过open函数打开，read/write函数完成读写，close函数关闭。

为什么微软的Windows没有在手机终端领域取得领先地位？
- [x]  http://tech.qq.com/a/20131013/000091.htm

>  苹果发布首款iphone打破了传统手机的架构，作为智能手机领域开先河的公司，在占有市场份额中具有一定的优势。微软在开始时没有认识到市场，等到后期开始复制iPhone操作系统及其经营模式，以及为时已晚。同时这一步的落后导致了，微软移动平台的开发人员不如苹果。一个好的操作系统，的优秀程度需要通过在它上面运行的应用程序体现出来。新的热门应用比如新游戏或社交网站出现的时候，至少需要过几个月才能看到兼容WP的版本。因此，购买WP手机的消费者必须接受这样一个事实，那就是他们能够使用的应用将是比较受限的，这也正是大多数消费者不愿意购买WP手机的重要原因之一。

你认为未来（10年内）的操作系统应该具有什么样的特征和功能？
- [x]  

>  在性能方面，操作系统应当随着硬件的不断提升而不断改进匹配硬件的水平，能够将硬件和应用软件充分的结合在一起。在用户方面，我认为可能会出现脑波交互、全息交互等更加便捷友好的交互方式。
