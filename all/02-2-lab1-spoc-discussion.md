...
>  建立一个中断描述符表（IDT），当一个中断产生的时候会有对应的终端号，比如时钟中断会有对应的中断号。用这个
终端号作为索引，设置中断描述符表对应索引的一项，指向中断服务例程的地址。这个地址中包括段选择子和偏移，段描述符存在
于全局描述表中，根据段选择子来查询段描述符。根据段描述符确定中断服务例程的基址，加上偏移量就形成了中断服务例程的地址
最后利用lidt指令，告知CPU完成中断描述符表的加载，和IDT的位置。以上完成了8259中断控制器的初始化，和IDT的初始化。然后进行外设的
初始化，对8253时钟的初始化，要使中断使能，使它能够每100个tick产生一个中断。
...
再实现一个可加载ELF格式文件的bootloader（再描述一下进一步要实现的关键步骤和需要注意的事项）...） (spoc)
- [x]  

> 第一步：实现一个能够显示字符串的bootloader，关键步骤是先将CPU从实模式切换到保护模式，
即通过修改A20地址线完成从实模式到保护模式的转换。然后要启动分段存储管理机制，只有在保护模式下才能使用分段存储管理机制。
其中需要注意4个关键内容：逻辑地址、段描述符（描述段的属性）、段描述符表（包含多个段描述符的“数组”）、
段选择子（段寄存器，用于定位段描述符表中表项的索引）。完成这些内容的建立和初始化，是bootloader的关键。

第二步：实现一个可加载ELF格式文件的bootloader，其需要完成的工作是从硬盘上加载并运行OS。LBA模式的PIO（Program IO）方式，
即所有的IO操作是通过CPU访问硬盘的IO地址寄存器完成，来访问硬盘。这一步实现需要注意理解ELF文件格式。
ELF格式是Linux系统下的一种常用目标文件(object file)格式，能够用于执行的可执行文件(executable file)，用于提供程序的进程映像，
加载的内存执行。
...
> 使用strace函数，其中当参数为-c，可以统计每一系统调用的所执行的时间,次数和出错的次数等信息. 

如何修改lab1, 实现一个可显示字符串"THU LAB1"且依然能够正确加载ucore OS的bootloader？
如果不能完成实现，请说明理由。
- [x]  

> 无法完成实现，因为原有bootloader的大小已经接近上限，再增加代码显示THU LAB1可能会超过代码大小的限制，引起错误。
...
对于ucore_lab中的labcodes/lab1,如果出现了reboot，死机，黑屏等现象，
请思考设计有效的调试方法来分析常在现背后的原因。
- [x]  
